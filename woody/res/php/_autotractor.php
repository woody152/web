...
function EchoAll()
{
    ...
    $strNepturn = GetBoldElement('海王星单独委托版3.29');
...
//    	'在小屏幕笔记本上，显示设置的'.GetInfoElement('缩放与布局').'中，'.GetInfoElement('更改文本、应用等项目的大小').'的选项缺省不是100%。这时AutoIt自带的WinGetPos函数不会跟着调整倍数，导致找不到验证码位置。',
    echo GetKnownBugs(array(
    						 ...
    						 '电脑上同时安装了海王星和双子星时可能会有异常，建议先卸载双子星，等海王星和autoit配合工作后再重新安装双子星。',
    						 '在基金概要文件那部分，浏览器会弹出框让选择打开或者下载，需要手工点一下，要不到不了下一步。这是要给PDF文件设置缺省打开的应用程序，给浏览器安装adobe的阅读PDF插件后能解决这个问题。在电脑上安装一下Adobe官方的免费PDF阅读器软件也可以解决这个问题。安装最后问是否设置为默认应用程序的时候要确认。另外也不要安装过多的PDF阅读器，有问题是优先确认电脑上只有一个PDF阅读器，把其它多余的都卸载。',
